...
func Validate(cfg *Config) error {
	...
	// Provider validation only if provider.interface_name is set (provider mode enabled)
	...
	// Client validation only if client.interface_name is set (client mode enabled)
	...
	// Cross-field validations that involve both provider and client only if both are configured
	if strings.TrimSpace(cfg.Provider.InterfaceName) != "" && strings.TrimSpace(cfg.Client.InterfaceName) != "" {
	...
	// Logging, Security, and other global validations (always applied)
