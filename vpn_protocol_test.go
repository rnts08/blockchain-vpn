...
func TestEncodeDecodePayload(t *testing.T) {
	...
	// Initialize public keys with some data to ensure they are valid length
	for i := range tests {
		for j := 0; j < wgtypes.KeyLen; j++ {
			tests[i].endpoint.PublicKey[j] = byte(i + j)
		}
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !bytes.Equal(tt.endpoint.PublicKey, decoded.PublicKey) {
				t.Errorf("PublicKey mismatch: got %x, want %x", decoded.PublicKey, tt.endpoint.PublicKey)
			}
		})
	}
}
