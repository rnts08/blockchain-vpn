...
apply-networking:
	...
	sudo iptables -t mangle -A OUTPUT -p udp --sport $(PROVIDER_PORT) -j MARK --set-mark $(MARK_ID)
	...
	# 2. Firewall Rules
	sudo iptables -A INPUT -p udp --dport $(PROVIDER_PORT) -j ACCEPT
...
remove-networking:
	...
	sudo iptables -D INPUT -p udp --dport $(PROVIDER_PORT) -j ACCEPT || echo "Rule not found"

	# 1. Policy Based Routing
	sudo ip rule del fwmark $(MARK_ID) lookup $(TABLE_ID) || echo "Rule not found"
	sudo iptables -t mangle -D OUTPUT -p udp --sport $(PROVIDER_PORT) -j MARK --set-mark $(MARK_ID) || echo "Rule not found"
...
check-networking:
	...
	@sudo iptables -t mangle -C OUTPUT -p udp --sport $(PROVIDER_PORT) -j MARK --set-mark $(MARK_ID) 2>/dev/null && echo "[OK] Output mark rule exists" || echo "[MISSING] Output mark rule"

	@echo "--- IPTables Filter ---"
	@sudo iptables -C INPUT -p udp --dport $(PROVIDER_PORT) -j ACCEPT 2>/dev/null && echo "[OK] Input accept rule exists" || echo "[MISSING] Input accept rule"
