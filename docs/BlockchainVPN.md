...
| **Price** | 8 | Cost in Satoshis per session (uint64). |
...
4.  **Connection:**
    ...
    *   Client creates a local TUN interface and establishes a TLS connection to the provider, verifying the peer certificate against the on-chain provider public key.
